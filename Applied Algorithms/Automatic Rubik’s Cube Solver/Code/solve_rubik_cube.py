...
def color_detect(color_value):
    ...
    if value == (1,0,0): # đỏ
        color_result = 'F'
    elif value == (1,1,0): # vàng
        color_result = 'R'
    elif value == (1, 0.5, 0): # cam
        color_result = 'B'
    elif value == (1, 1, 1): # trắng
        color_result = 'L'
    elif value == (0, 0, 1): # xanh nước biển
        color_result = 'U'
    elif value == (0, 1, 0): # xanh lá cây
        color_result = 'D'
    return color_result
# Hàm giải
def decode_postion(cube):
    '''F = đỏ, R = vàng, B = cam, L = trắng, U = xanh nước biển, D = xanh lá cây'''
    value = ['0']*54
    for tile in cube:
