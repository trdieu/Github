...
class Rubic_Cube():
    ...
    def __init__(self):
        ...
        self.moves = []
    # HÀM CẬP NHẬT VỊ TRÍ CÁC MẶT
    def reset_positions(self):
        self.positions = {'front': [], 'right': [], 'back': [], 'left': [], 'top': [], 'bottom': []}
        for tile in self.tiles: # phân loại các ô màu dựa vào tọa độ x, y, z
            if tile.pos.z > 0.4:
    ...
    def move(self):
        ...
        if self.rotate[0] == None and len(self.moves) > 0:
            ...
            self.moves.pop(0)
    # HÀM XÁO TRỘN, LỜI GIẢI VÀ GIẢI TỰ ĐỘNG
    def scramble(self):
        possible_moves = ["F","R","B","L","U","D","F'","R'","B'","L'","U'","D'"]
        for i in range(25):
    ...
    def control(self):
        ...
        button(bind=self.scramble, text='Xáo trộn')
        button(bind=self.solution, text='Gợi ý giải')
        button(bind=self.solve, text='Giải tự động!')
    def update(self):
        rate(60)
        self.animations()
        self.move()
